	...
	double*& bvec)
{
	...
	for( j = 0; j < sz; j++ )
	{
		unsigned char r = ubuff[3*j];
		unsigned char g = ubuff[3*j+1];
		unsigned char b = ubuff[3*j+2];
	...
	double**& bvec)
{
	...
	for( d = 0; d < m_depth; d++ )
	{
		for( j = 0; j < sz; j++ )
		{
			unsigned char r = ubuff[d][3*j];
			unsigned char g = ubuff[d][3*j+1];
			unsigned char b = ubuff[d][3*j+2];
...
///
/// The input parameter ubuff conains RGB values of input image.  
/// RGBRGB...for 24 bit color. (Modified by Shan An <anshan.tju@gmail.com>)
///
...
//===========================================================================
/// Modified for JPEG format data. RGBRGB...for 24 bit color. Shan An.
//===========================================================================
...
/// RGBRGB...for 24 bit color. (Modified by Shan An <anshan.tju@gmail.com>)
///
/// The RGB values are accessed from (and packed into) the unsigned char
/// using bitwise operators as can be seen in the function DoRGBtoLABConversion().
///
/// compactness value depends on the input pixels values. For instance, if
/// the input is greyscale with values ranging from 0-100, then a compactness
...
///
/// unsigned char the_Rcomponent_pixel_i_want = ubuffvec[2][3*(3*width + 36)]
/// unsigned char the_Gcomponent_pixel_i_want = ubuffvec[2][3*(3*width + 36) + 1]
/// unsigned char the_Bcomponent_pixel_i_want = ubuffvec[2][3*(3*width + 36) + 2]
///
/// The input parameter ubuff conains RGB values of input image. 
/// RGBRGB...for 24 bit color.  (Modified by Shan An <anshan.tju@gmail.com>)
///
